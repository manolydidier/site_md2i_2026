...
const DEFAULT_CONFIG: ProspectConfig = {
  isActive: false,
  delayValue: 0,
  delayUnit: "MINUTES",
...
function renderPreview(template: string) {
  return cleanEmailHtml(template || "")
    ...
    .replaceAll(
      "{{prospectLink}}",
      "https://md2i.eu?preview=prospect-link"
    );
}

function getDelayLabel(delayValue: number, delayUnit: DelayUnit) {
  if (delayValue === 0) {
    return "Envoyé immédiatement quand le contact devient prospect";
  }
  ...
  return `Envoyé ${delayValue} ${unitLabels[delayUnit]} après le passage en prospect`;
}
...
export default function ProspectEmailAutomationPage() {
  ...
  const loadProspectConfig = async () => {
    ...
    try {
      const res = await fetch("/api/email-automations/prospect", {
        method: "GET",
        cache: "no-store",
      });
      ...
      const nextConfig: ProspectConfig = {
  ...
  const updateConfig = <K extends keyof ProspectConfig>(
    key: K,
    value: ProspectConfig[K]
  ) => {
    setConfig((prev) => ({
      ...prev,
      [key]: value,
  ...
  const handleSave = async () => {
    if (isInvalidActiveConfig) {
      setMessage({
        type: "error",
        text: "Choisissez une campagne brouillon avant d’activer l’automatisation de prospection.",
      });
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Paramétrage de l’email de prospection sauvegardé.",
      });
    } catch (err) {
      setMessage({
        type: "error",
      ...
      <header style={s.header}>
        <div>
          ...
          <div style={s.titleRow}>
            ...
            <div>
              <h1 style={s.title}>Email de prospection</h1>
              <p style={s.subtitle}>
                Choisissez la campagne envoyée automatiquement quand un contact
                devient prospect.
              </p>
            </div>
          </div>
        ...
        <section style={s.card}>
          <div style={s.cardHead}>
            <div>
              ...
              <p style={s.cardSubtitle}>
                Cette automatisation utilise le déclencheur
          ...
          <div style={s.fieldGroup}>
            ...
            <p style={s.helpTextPlain}>
              Crée une campagne brouillon nommée par exemple “Email de
              prospection”. Son contenu sera envoyé quand un contact devient
              prospect.
            </p>
          </div>
          ...
          {selectedCampaign && (
            <div style={s.selectedBox}>
              ...
              <span>Reply-to : {selectedCampaign.replyTo || "Non défini"}</span>
              <span>
          ...
          {draftCampaigns.length === 0 && (
            <div style={s.warningBox}>
              ...
              brouillon nommée “Email de prospection”.
            </div>
          )}
          ...
          <div style={s.infoBox}>
            ...
            <p>
              Cette automatisation se déclenche quand le contact passe au statut{" "}
              <strong>PROSPECT</strong>. Si tu utilises le bouton{" "}
              <strong>{"{{prospectLink}}"}</strong> dans l’email de bienvenue,
              le clic peut automatiquement transformer le contact en prospect.
            </p>
          </div>
        </section>
