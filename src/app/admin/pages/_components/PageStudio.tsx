...
import PageMetaSidebar, { PageMeta } from './PageMetaSidebar'
...
export default function PageStudio({ mode, pageId, canUpdate = true }: PageStudioProps) {
 ...
 const [meta, setMeta] = useState<PageMeta>({
  ...
  async (editor: Editor) => {
    ...
    try {
      ...
      if (page.gjsComponents) editor.setComponents(page.gjsComponents)
      else if (page.gjsHtml) editor.setComponents(page.gjsHtml)

      if (page.gjsStyles) editor.setStyle(page.gjsStyles)

      setCodeJs(page.gjsJs ?? '')
      ensureBaseBlocksCss(editor)

      setTimeout(() => {
        runCanvasJs(page.gjsJs ?? '')
  ...
  [pageId, readPageStyles, syncCodeFieldsSilently]
)
  ...
  }, [installCanvasLinkGuard, loadPage, mode, readPageStyles, syncLinkConfigFromSelection]);
  ...
  useEffect(() => {
    if (!isReady) return;
    const draft = loadDraft("page", draftId);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      saveDraft("page", draftId, snapshot);
      pushHistoryEntry("page", draftId, snapshot);
  ...
  const openTab = (tab: PanelTab) => {
    ...
    if (tab === "history") setHistoryEntries(listHistory("page", draftId));
    ...
    <div className="ed-root">
      <header className="ed-topbar">
        <div className="ed-topbar__left">
          <button className="ed-icon-btn" onClick={() => router.push("/admin/pages")} title="Retour">
