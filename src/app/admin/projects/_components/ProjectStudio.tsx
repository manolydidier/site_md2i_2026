...
import ProjectMetaSidebar, { ProjectMeta } from './ProjectMetaSidebar'
...
export default function ProjectStudio({ mode, projectId, canUpdate = true }: ProjectStudioProps) {
 ...
 const [meta, setMeta] = useState<ProjectMeta>({
  ...
  async (editor: Editor) => {
    ...
    try {
      ...
      if (project.gjsComponents) editor.setComponents(project.gjsComponents)
      else if (project.gjsHtml) editor.setComponents(project.gjsHtml)

      if (project.gjsStyles) editor.setStyle(project.gjsStyles)

      setCodeJs(project.gjsJs ?? '')
      ensureBaseBlocksCss(editor)

      setTimeout(() => {
        runCanvasJs(project.gjsJs ?? '')
  ...
  [projectId, readPageStyles, syncCodeFieldsSilently]
)
  ...
  const installCanvasLinkGuard = useCallback((editor: Editor, frameWindow?: Window | null) => {
    ...
    const syncDocumentMode = () => {
      const enabled = linksEnabledRef.current;
      ...
      doc.documentElement.setAttribute("data-md2i-links", enabled ? "enabled" : "disabled");
    };

    const getLinkElement = (target: EventTarget | null): Element | null => {
      // Ne pas utiliser `target instanceof Element` ici : le canvas GrapesJS
      // vit dans une iframe, donc ses éléments appartiennent à un autre realm/window.
      ...
      return candidate.closest('a[href], [role="link"], button');
    };
  ...
  const setCanvasLinksMode = useCallback((enabled: boolean) => {
    ...
    // Réinstalle le guard si le document du canvas a été recréé entre-temps.
    installCanvasLinkGuard(editor, editor.Canvas.getWindow());
    doc.documentElement.classList.toggle("md2i-links-disabled", !enabled);
    doc.documentElement.classList.toggle("md2i-links-enabled", enabled);
    doc.documentElement.setAttribute("data-md2i-links", enabled ? "enabled" : "disabled");
  }, [installCanvasLinkGuard]);
  ...
  }, [installCanvasLinkGuard, loadProject, mode, readPageStyles, syncLinkConfigFromSelection]);
  ...
  useEffect(() => {
    if (!isReady) return;
    const draft = loadDraft("project", draftId);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      saveDraft("project", draftId, snapshot);
      pushHistoryEntry("project", draftId, snapshot);
  ...
  const openTab = (tab: PanelTab) => {
    ...
    if (tab === "history") setHistoryEntries(listHistory("project", draftId));
    ...
    <div className="ed-root">
      <header className="ed-topbar">
        <div className="ed-topbar__left">
          <button className="ed-icon-btn" onClick={() => router.push("/admin/projects")} title="Retour">
