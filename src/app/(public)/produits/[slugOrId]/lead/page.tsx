...
/* ------------------------------------------------------------------ */
/* Constantes statiques (hoistées hors du composant)                   */
/* ------------------------------------------------------------------ */

const PLACEHOLDER_IMAGE = "/placeholder-reference.svg";
...
function formatPrice(
  ...
  // Comportement conservé : si la valeur n'est pas numérique, on renvoie
  // la chaîne brute plutôt que de la masquer.
  return Number.isFinite(numeric)
    ? `${PRICE_FORMATTER.format(numeric)} Ar`
    : raw;
}

function safeImage(value?: string | null) {
  return value?.trim() || PLACEHOLDER_IMAGE;
}
...
/* ------------------------------------------------------------------ */
/*
 * NOTE PERFORMANCE : le `@import` Google Fonts ci-dessous est
 * render-blocking et réinjecté à chaque rendu. La bonne pratique
 * Next.js est de charger Inter via `next/font/google` dans le layout :
 *
 *   import { Inter } from "next/font/google";
 *   const inter = Inter({ subsets: ["latin"], variable: "--lead-font-body" });
 *
 * puis d'appliquer `inter.variable` sur <html> et de retirer ce @import.
 */
const LEAD_PAGE_STYLES = `
