...
function normalizeDynamicTriggerFromBody(body: any) {
  ...
  const conditionAllowedEvents: TriggerEvent[] = [
    "CONTACT_CREATED",
    "CONTACT_UPDATED",
  ];

  if (!conditionAllowedEvents.includes(triggerEvent)) {
    conditionField = "NONE";
    conditionOperator = "ALWAYS";
    conditionValue = null;
  }
...
function validateDynamicCondition({
  ...
  const changeOperators: ConditionOperator[] = [
    "CHANGED",
    "CHANGED_TO",
    "CHANGED_FROM",
  ];
  ...
  if (
    triggerEvent === "CONTACT_CREATED" &&
    changeOperators.includes(conditionOperator)
  ) {
...
export async function GET(
  ...
  return NextResponse.json({
    ...
    // Anciennes options conservées pour compatibilité.
    triggerOptions: LEGACY_TRIGGERS,

    // Nouvelles options dynamiques.
...
export async function PUT(
  ...
  const firstStep =
    existing.steps.find((step) => step.isActive) || existing.steps[0];
  ...
  const reloaded = await prisma.emailAutomation.findFirst({
    where: {
      id,
      userId,
    },
...
export async function DELETE(
  ...
  await prisma.$transaction(async (tx) => {
    if (runIds.length > 0) {
      await tx.emailAutomationEmail.updateMany({
        where: {
          userId,
          runId: {
            in: runIds,
      ...
      await tx.emailAutomationRun.updateMany({
        where: {
          userId,
          id: {
            in: runIds,
    ...
    await tx.emailAutomation.update({
      ...
      data: {
        isActive: false,
        name: `[SUPPRIMÉE] ${new Date().toISOString()} - ${id}`,
      },
    });
  });
